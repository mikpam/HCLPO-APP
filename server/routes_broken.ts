...
totalPrice: ${item.totalPrice || 0}`;
  ...
  // UNIFIED AUTO PROCESSING ENDPOINT - Main route through validation system
  app.post("/api/processing/process-auto", async (req, res) => {
    try {
      ...
      updateProcessingStatus({
        isProcessing: true,
        currentStep: "email_preprocessing",
        currentEmail: "Scanning for new emails...",
        emailNumber: 0,
        totalEmails: 0
      });
      ...
      console.log(`\n🔄 AUTO PROCESSING: Processing single email`);
      console.log(`📧 EMAIL: "${messageToProcess.subject}"`);
      console.log(`   └─ From: ${messageToProcess.sender}`);
      console.log(`   └─ Attachments: ${messageToProcess.attachments.length}`);

      // Process the email using the complete validation system
      const result = await processEmailThroughValidationSystem(messageToProcess, updateProcessingStatus);

      // 🔥 RESET STATUS - Always reset regardless of success/failure
      updateProcessingStatus({
        isProcessing: false,
        currentStep: "",
        currentEmail: "",
